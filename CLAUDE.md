...
- **Base / overlay model** (`determine_active_scenarios`, `_is_base_scenario`, `_resolve_applies_to`): scenarios are either *bases* (`is_base: true`, or legacy `source: default`) or *overlays*. Exactly one base is active per run — the selected `is_base` scenario, else the top-level `default_base`; bases never mix. Overlays declare `applies_to` (a string or list of base values; omitted → inherits `default_base`) and join the merge only if the active base is one of their `applies_to` bases (OR semantics); otherwise they are silently excluded. An overlay listing multiple bases is reused across each base's separate run — it does not merge the bases together. `applies_to` is normalized to a list in `ScenarioConfig.from_dict`. The active base is forced to `priority = 9999` (lowest layer). `validate_config_scenarios` enforces that every `applies_to` value and `default_base` reference real `is_base` scenarios and that no scenario is both `is_base` and declares `applies_to`. This lets e.g. `tvm` fully replace `general_cluster` as the base rather than stacking on it.
